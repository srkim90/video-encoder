...
def add_gpx_line(fd, lat, lon, altitude):
    lat = lat * (37.4131190 / 446355899.0)
    lon = lon * (127.1346390 / 1516775322)
    line = '%s<trkpt lat="%0.14f" lon="%0.14f">\n%s<ele>%0.1f</ele>\n%s</trkpt>\n' % (
        "\t" * 3,
        lat, lon,
        "\t" * 4,
        altitude,
...
def write_fitfile_to_csv(
    ...
    for fitfile in fitfiles:
        ...
        if event_type is None:
            event_type = 'other'
        output_file = (timestamp  + datetime.timedelta(hours=9)).strftime('%Y-%m-%d_%H-%M-%S')
            ...
            for field in fields:
                if not changed_tz and field.name in ['position_lat','position_long', 'start_position_lat','start_position_long']:
                    ...
                    if position_lat is not None and position_long is not None:
                        changed_tz = True
                        try:
                            tz_name = "Asia/Seoul"#tzwhere.tzNameAt(position_lat, position_long)
                        except KeyError:
                            tz_name = None
                        if tz_name is None:
                ...
                if field.name in all_allowed_fields:
                    ...
                    if field.name=='timestamp':
                        mdata[field.name] = field.value + datetime.timedelta(hours=9)
                        #UTC.localize(field.value).astimezone(local_tz) + datetime.timedelta(hours=8)
                    else:
    ...
    if output_file == None:
        return None
    print(output_file)
    output_full_file = os.path.join(fit_processed_csv_dir, output_file + ".gpx")
    with open(output_full_file, 'w') as f:
        f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
        f.write('<gpx xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.1" xmlns="http://www.topografix.com/GPX/1/1" creator="srkim.co.kr">\n')
        f.write('%s<trk>\n%s<trkseg>\n' % ('\t' * 1, '\t' * 2))
        for entry in data:
            enhanced_altitude = entry["enhanced_altitude"]
            position_lat = entry["position_lat"]
            position_long = entry["position_long"]
            add_gpx_line(f, position_lat, position_long, enhanced_altitude)
        f.write('%s</trkseg>\n%s</trk>\n</gpx>' % ('\t' * 2, '\t' * 1))
    output_full_file = os.path.join(fit_processed_csv_dir, output_file + ".json")
    with open(output_full_file, 'w') as f:
        data2 = []
        distance = 0
        enhanced_speed = 0
        ...
        for entry in data:
            ...
            position_lat = entry["position_lat"]
            position_long = entry["position_long"]
            lat = position_lat * (37.4131190 / 446355899.0)
            lon = position_long * (127.1346390 / 1516775322)
            yyyymmdd = entry["timestamp"].strftime('%Y-%m-%d')
            hhmmss = entry["timestamp"].strftime('%H-%M-%S')
            try:
                distance = entry["distance"] / 1000.0
                enhanced_speed = entry["enhanced_speed"]
                speed = entry["speed"]
                cadence = entry["cadence"]
                temperature = entry["temperature"]
                fractional_cadence = entry["fractional_cadence"]
        ...
        json_data = json.dumps({"data" : data2}, indent=4)
        f.write(json_data)
    return output_full_file
